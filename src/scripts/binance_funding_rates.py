...
class FundingAnalyzer:
    ...
    async def get_historical_funding_rates(self, start_date: datetime, end_date: datetime, 
        ...
        try:
            ...
            # Forward fill Binance rates to match HL frequency
            if not hl_df.empty and not binance_df.empty:
                # Resample Binance data to hourly and forward fill
                binance_df = binance_df.set_index('timestamp')
                binance_df = binance_df.resample('1H').ffill()
                binance_df = binance_df.reset_index()
                
                # Merge HL and Binance data
                funding_df = pd.merge_asof(
                    hl_df.sort_values('timestamp'),
                ...
                funding_df = funding_df[
                    (funding_df['timestamp'] >= pd.Timestamp(start_date, tz='UTC')) &
                    (funding_df['timestamp'] <= pd.Timestamp(end_date, tz='UTC'))
                ]
                
                logger.info(f"\n=== Final Funding Data ===")
                logger.info(f"Data shape: {funding_df.shape}")
                logger.info(f"Sample data:\n{funding_df.head().to_string()}")
                logger.info(f"Value ranges:")
                logger.info(f"HL rates: {funding_df['hl_rate'].min():.6f} to {funding_df['hl_rate'].max():.6f}")
                logger.info(f"Binance rates: {funding_df['binance_rate'].min():.6f} to {funding_df['binance_rate'].max():.6f}")
    ...
    def get_price_history(self, symbol: str, start_date: datetime, end_date: datetime) -> pd.DataFrame:
        ...
        try:
            ...
            response = (self.supabase
                .schema('crypto_historical')  # Correctly set schema
                .from_('price_history')  # Use only the table name
                .select("*")
                .eq('symbol', symbol)
                .gte('datetime', start_date.isoformat())
                .lte('datetime', end_date.isoformat())
                .order('datetime', desc=False)
                .execute())  # No 'headers' argument here
    ...
    async def analyze_symbol(self, symbol: str, start_date: datetime, end_date: datetime, 
        ...
        try:
            ...
            # Create timestamp column with consistent UTC timezone
            price_df['timestamp'] = pd.to_datetime(price_df['datetime'], utc=True)
            ...
            # Debug logging
            logger.info(f"Funding timestamp type: {funding_df['timestamp'].dtype}")
            logger.info(f"Price timestamp type: {price_df['timestamp'].dtype}")
            logger.info(f"Sample funding timestamps: {funding_df['timestamp'].head()}")
            logger.info(f"Sample price timestamps: {price_df['timestamp'].head()}")
            ...
            # Log merge results
            logger.info(f"Funding data shape: {funding_df.shape}")
            logger.info(f"Price data shape: {price_df.shape}")
            logger.info(f"Merged data shape: {merged_df.shape}")
            
            # Log column names to ensure no duplicates
    ...
    def create_differential_plot(self, df: pd.DataFrame, symbol: str, significant_threshold: float) -> go.Figure:
        """Create plot showing funding rate differentials over time"""
        fig = go.Figure()
        
        # Plot all differentials
        ...
        fig.add_hline(y=significant_threshold, line_dash="dash", line_color="red", 
                     annotation_text=f"Upper threshold: {significant_threshold:.6f}")
        fig.add_hline(y=-significant_threshold, line_dash="dash", line_color="red",
                     annotation_text=f"Lower threshold: {-significant_threshold:.6f}")
        
        fig.update_layout(
            title=f'Funding Rate Differential for {symbol} (8h intervals)',
            xaxis_title='Date',
    ...
    def create_return_analysis(self, df: pd.DataFrame, window: int) -> go.Figure:
        """Create detailed return analysis plot"""
        fig = go.Figure()
        
        # Scatter plot of differentials vs returns
        fig.add_trace(go.Scatter(
            x=df['rate_diff'],
        ...
        # Add trendline
        z = np.polyfit(df['rate_diff'], df[f'return_{window}h'], 1)
        p = np.poly1d(z)
        fig.add_trace(go.Scatter(
            x=df['rate_diff'],
            y=p(df['rate_diff']),
        ...
        fig.update_layout(
            title=f'Rate Differential vs {window}h Returns (Significant Periods)',
