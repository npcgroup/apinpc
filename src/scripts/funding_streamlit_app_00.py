...
def calculate_opportunity_score(row):
    ...
    try:
        ...
        # Adjust based on exchange (prefer Hyperliquid)
        if row['exchange'] == 'Hyperliquid':
            score *= 1.2
            
        # Adjust based on direction if available
        if 'direction' in row and row['direction'] != 'neutral':
            score *= 1.1
    ...
    try:
        ...
        if isinstance(df, pd.DataFrame) and not df.empty:
            ...
            try:
                ...
                # Calculate rate differences and opportunity score
